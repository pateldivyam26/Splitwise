...
async function handleExpenseCreation(req, res) {
  try {
    ...
    // Create and Save Expense
    const expenseData = { expenseName, payer, expenseDate, description, amount, groupId, payerName, participants, splitType, category };
...
async function handleExpenseUpdate(req, res) {
  try {
    ...
    // Update Expense
    const updatedExpense = await Expense.findByIdAndUpdate(expenseId, { $set: req.body.expenseData }, { new: true });
